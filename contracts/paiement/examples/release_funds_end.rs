...
pub const WINNERS_SHARE: Decimal = Decimal::permille(875);
pub const WINNER_SHARE: Decimal = Decimal::percent(10);
...
pub fn main() -> anyhow::Result<()> {
    ...
    todo!("{:?} - {:?}", participants, winner);

    let chain = Daemon::builder(NEUTRON_1).build()?;
    let balance =
        chain.balance(&Addr::unchecked(TREASURY_ADDR), Some(TOKEN.to_string()))?[0].clone();

    let community_pool_amount =
        Uint128::from(balance.amount.u128()).mul_floor(COMMMUNITY_POOL_SHARE);
    let total_available_amount = Uint128::from(balance.amount.u128()).mul_floor(WINNER_SHARE);
    let winner_amount = total_available_amount / Uint128::from(10u128);
    let participants_amount = total_available_amount - winner_amount;
    ...
    let share = participants_amount / Uint128::from(participants.len() as u128);

    for (i, chunk) in participants.chunks(CHUNK_SIZE).enumerate() {
        let mut participant_msgs = vec![];
...
async fn query_all_participants() -> anyhow::Result<Vec<String>> {
    ...
    let participants = sqlx::query_as!(
        Participant,
        "SELECT accounts.userName as participant FROM memories JOIN accounts ON memories.\"userId\" = accounts.id WHERE type = 'messages' AND memories.\"agentId\" != memories.\"userId\" ",
    )
    .fetch_all(&pool)
    .await?;
